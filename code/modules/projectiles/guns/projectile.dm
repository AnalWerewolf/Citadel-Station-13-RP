...
/obj/item/weapon/gun/projectile/proc/load_ammo(var/obj/item/A, mob/user)
	if(istype(A, /obj/item/ammo_magazine))
		...
		switch(method_for_this_load)
			if(MAGAZINE)
				...
				if(tac_reloads && ammo_magazine)
					...
					to_chat(user,"<span class='notice'>You tactically reload your [src] with [AM]!")
				else if(ammo_magazine)
					to_chat(user,"<span class='warning'>[src] already has a magazine loaded.") //already a magazine here
					return
				if(!(load_method & AM.mag_type) || caliber != AM.caliber || allowed_magazines && !is_type_in_list(A, allowed_magazines))
					to_chat(user,"<span class='warning'>[AM] won't fit into the magwell.") //wrong magazine
					return
			...
			if(SPEEDLOADER)
				...
				var/count = 0
				if(AM.reload_delay)
					to_chat(user, "<span class='notice'>\It takes some time to reload [src] with [AM]...")
				if (do_after(user, AM.reload_delay, user))
					for(var/obj/item/ammo_casing/C in AM.stored_ammo)
						...
						if(C.caliber == caliber)
							...
							count++
				if(count)
					user.visible_message("[user] reloads [src].", "<span class='notice'>\You load [count] round\s into [src].")
					if(reload_sound) playsound(src.loc, reload_sound, 75, 1)
					cock_gun(user)
				update_firemode()
		AM.update_icon()
		...
		if(C.reload_delay)
			to_chat(user, "<span class='notice'>\It takes some time to reload [src] with [C]...")
		...
		update_firemode()
		user.visible_message("[user] inserts \a [C] into [src].", "<span class='notice'>\You insert \a [C] into [src].")
	...
	if(ammo_magazine)
		...
		if(allow_dump && (load_method & SPEEDLOADER))
			...
			if(T)
				...
				loaded.Cut()
			if(count)
				user.visible_message("[user] unloads [src].", "<span class='notice'>\You unload [count] round\s from [src].")
				if(bulletinsert_sound) playsound(src.loc, bulletinsert_sound, 75, 1)
		else if(load_method & SINGLE_CASING)
			var/obj/item/ammo_casing/C = loaded[loaded.len]
			loaded.len--
			user.put_in_hands(C)
			user.visible_message("[user] removes \a [C] from [src].", "<span class='notice'>\You remove \a [C] from [src].")
			if(bulletinsert_sound) playsound(src.loc, bulletinsert_sound, 75, 1)
	else
...
/obj/item/weapon/gun/projectile/afterattack(atom/A, mob/living/user)
	...
	if(auto_eject && ammo_magazine && ammo_magazine.stored_ammo && !ammo_magazine.stored_ammo.len)
		...
		user.visible_message(
			...
			"<span class='notice'>\[ammo_magazine] falls out and clatters on the floor!"
			)
		if(auto_eject_sound)
